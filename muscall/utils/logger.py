...
class Logger:
    ...
    def init_pretrain_logger(self):
        if self.config.env.experiment_id is None:
            self.experiment_id = self.get_timestamp()+f"_{self.config.model_config.midi.model_name}_loadckpt_"+str(self.config.model_config.midi.load_ckpt) # 現在時刻を実験idとする
